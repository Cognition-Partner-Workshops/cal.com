...
function isPasswordValid(
  ...
  breakdown: boolean,
  strict?: boolean
): { caplow: boolean; num: boolean; min: boolean; admin_min: boolean; max: boolean; special: boolean };
function isPasswordValid(password: string, breakdown?: boolean, strict?: boolean) {
  let cap = false, // Has uppercase characters
    ...
    admin_min = false,
    max = false, // Maximum 128 characters
  ...
  if (password.length <= 128) max = true;
  if (password.match(/\d/)) num = true;
  if (password.match(/[a-z]/)) low = true;
  if (password.match(/[A-Z]/)) cap = true;
  if (password.match(/[!@#$%^&*()_+\-=[\]{}|;:,.<>?]/)) special = true;

  if (!breakdown) return cap && low && num && min && max && special && (strict ? admin_min : true);

  let errors: Record<string, boolean> = { caplow: cap && low, num, min, max, special };
